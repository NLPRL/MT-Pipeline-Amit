...
def load_data(chunking_file_path, min_freq=1):
    ...
    chunk_tags = sorted(list(set(row[7] for sample in file_train for row in sample)))
    ...
    train = _process_data(file_train, vocab, pos_tags, chunk_tags,characters,features)
    return train, (vocab, pos_tags, chunk_tags,characters,features)
def _parse_data(fh):
    string = fh.read()
    # print(string)
    data = []
...
def pad_words(l,max_len_char):
    ...
    l=l1+l
    return l
def _process_data(data, vocab, pos_tags, chunk_tags, characters, features,onehot=False):
    global max_len
    if max_len is None:
    ...
    y_chunk = [[chunk_tags.index(w[7]) for w in s] for s in data]
    ...
    return x, y_pos, y_chunk, y_feature
...
pos_word=(Embedding(len(pos_tags), 2*(EMBED_DIM//5), mask_zero=True))(pos_in)
# feature embedding
...
o=(crf)(o)
model=Model(input=[word_in,char_in,pos_in,f_in],output=o)
model.compile('adam', loss=crf_loss, metrics=[crf_viterbi_accuracy])
model.fit([X_train,np.array(X_char_train).reshape(len(X_char_train),max_len,max_len_char),x_pos_train.reshape(len(x_pos_train),max_len),x_feature_train.reshape(len(x_feature_train),max_len,features)],np.array(y_train).reshape(len(y_train), max_len, 1),batch_size=32, epochs=EPOCHS, validation_split=0.1, verbose=1)
#validation
l4=[0]*max_len_char
l4=numpy.asarray(l4)
