...
def main(test_input_word, test_feature_words):
    s2s = torch.load('/home/amit-nlprl/Documents/samparkPipeline/Machine-Translation-System-master/saved2.pt',
                     map_location=device)
    a=list(test_input_word)
    # a.append(list(test_input_word))
    a.append('<eos>')
    a.insert(0, '<sos>')
    sent_list = []
    sent_list.append(a)
    ...
    #
    #
    # word_encode_list1 = sentence_encoderTest(a, char_to_index)
    # print(word_encode_list1)
    # feat_encode_list1 = sentence_padFeat(sentence_encoderTestFeat(test_feature_words, feature_to_index))
    # print(feat_encode_list1)
    #
    # test_word = torch.tensor(word_encode_list1).view(1, -1).to(device)
    # print(test_word)
    # test_feat = torch.FloatTensor(feat_encode_list1).view(1, -1).to(device)
    # print(test_feat)
    # s2s.test_model(test_word, test_feat, device)
    # test_words_list = []
    # test_words_list.append(test_input_word)
    # words_list1 = boundary_appender(test_words_list)
    test_feature_list=[]
    test_feature_list.append(test_feature_words)
    # print(test_feature_list)
    feature_list = [feat.split(';') for feat in test_feature_list]
    print(feature_list)
    # extra_feature_list=[]
    # extra_feature_list.append(test_feature_list)
    # tempo = test_inflection_lists
    # inflection_list1 = boundary_appender(tempo)

    word_encode_list1 = sentence_encoder(sent_list, char_to_index)
    # inflection_encode_list1 = sentence_encoder(inflection_list1, char_to_index)

    feat_encode_list1 = sentence_pad(sentence_encoder(feature_list, feature_to_index), 5)
    ...
    for index in range(0, len(word_encode_list1)):
        ...
        predic_out = s2s.test_model(test_word, test_feat, device)
