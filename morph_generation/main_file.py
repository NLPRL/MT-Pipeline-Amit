...
file_name = BASE_DIR+'hindi-train-medium'
evaluate_file = BASE_DIR+'hindi-dev'
...
# s2s.train()
# torch.save(s2s,'saved.pt')


# encoder   = EncoderLSTM(vocab_size, embedding_size, encoder_hid_dem,decoder_hid_dem,encoder_bidirectional,device).to(device)              # (58, 300, 150)
# attention = Attention(encoder_hid_dem,decoder_hid_dem,encoder_bidirectional)
# decoder   = DecoderLSTM(decoder_hid_dem, encoder_hid_dem, vocab_size,embedding_size,attention,decoder_input_size,linear_input_size,decoder_birectional,device).to(device)          # (606, 300, 58)
# encoder_optimizer = torch.optim.Adam(encoder.parameters(), lr=learning_rate)
# decoder_optimizer = torch.optim.Adam(decoder.parameters(), lr=learning_rate)

"""**Helper code to above memory full in colab**"""
...
# train_acc, train_loss, val_acc, val_loss = train_iter(encoder, decoder, word_encode_list, inf_encode_list, feat_encode_list, test_word_encode_list, test_inf_encode_list, test_feat_encode_list, device,batch_size=BATCH_SIZE, epoch_size=EPOCH_SIZE)
...
#     print(param_tensor, "\t", decoder.state_dict()[param_tensor])
# print("attention's state_dict:")
# for param_tensor in attention.state_dict():
#     print(param_tensor, "\t", attention.state_dict()[param_tensor])
...
# torch.save(attention, 'attention_model.pt')
# torch.save(encoder_optimizer, 'encoder_optimizer.pt')
# torch.save(decoder_optimizer, 'decoder_optimizer.pt')
# encoder = torch.load('encoder_model.pt')
# decoder = torch.load('decoder_model.pt')
# attention = torch.load('attention_model.pt')
# encoder_optimizer = torch.load('encoder_optimizer.pt')
# decoder_optimizer = torch.load('decoder_optimizer.pt')
# torch.save(encoder.state_dict(), 'encoder_'+file_name+'.pt')
# torch.save(decoder.state_dict(), 'decoder_'+file_name+'.pt')
# torch.save(attention.state_dict(),'attention_'+file_name+'.pt')
# torch.save(encoder_optimizer.state_dict(),'encoder_optimizer'+file_name+'.pt')
# torch.save(decoder_optimizer.state_dict(),'decoder_optimizer'+file_name+'.pt')
...
#     'attention_states':attention.state_dict(),
#     'encoder_optimizer_states':encoder_optimizer.state_dict(),
#     'decoder_optimizer_states':decoder_optimizer.state_dict(),
# },'saved.pt')

# dic = torch.load('saved.pt')

# encoder = dic['encoder']
# decoder = dic['decoder']
# attention = dic['attention']
# encoder_optimizer = dic['encoder_optimizer']
# decoder_optimizer = dic['decoder_optimizer']

# encoder.load_state_dict(dic['encoder_states'])
# decoder.load_state_dict(dic['decoder_states'])
# attention.load_state_dict(dic['attention_states'])
# encoder_optimizer.load_state_dict(dic['encoder_optimizer_states'])
# decoder_optimizer.load_state_dict(dic['decoder_optimizer_states'])

# encoder.eval()
# attention.eval()
# decoder.eval()

# encoder.load_state_dict(torch.load('encoder_'+file_name+'.pt'))
# decoder.load_state_dict(torch.load('decoder_'+file_name+'.pt'))
# attention.load_state_dict(torch.load('attention_'+file_name+'.pt'))
# encoder_optimizer.load_state_dict(torch.load('encoder_optimizer'+file_name+'.pt'))
# decoder_optimizer.load_state_dict(torch.load('decoder_optimizer'+file_name+'.pt'))
...
# test_word, test_label, max_length = source_taget_equal_length_maker(word_encode_list1, inflection_encode_list1)
# test_word = torch.tensor(test_word).to(device)
# test_label = torch.tensor(test_label).to(device)
# test_feat = torch.FloatTensor(feat_encode_list1).to(device)

# word_pad_list1 = sentence_pad(words_list1, max_word_length)

"""**Function to test model**"""

# def test_model(encoder, decoder,test_input_word, test_feature_words,inflection_encode_list1,device,max_word_length=25):
#     batch_size = test_input_word.size()[0]

#     initial_word = encoder.embedding_layer(torch.tensor(char_to_index['<sos>']).view(1, -1).to(device)).repeat(batch_size, 1, 1)

#     encoder_outputs,hidden = encoder(test_input_word)
...
# def evaluate(encoder, decoder, sentence, sentence_feat):
#     # hidden = decoder.init_hidden(1)
#     # print(sentence.size())
#     # print(sentence_feat.size())
#     decoder_out = []
#     initial_word = encoder.embedding_layer(torch.tensor(char_to_index['<sos>']).view(1, -1).cuda())
#     # initial_word = encoder.embedding_layer(torch.tensor(12).view(1, -1).cuda())

#     encoder_outputs,hidden = encoder(sentence)
#     features = sentence_feat[:,:]


#     for idx in range(25):
#         # word = lstm_out[:, idx, :].unsqueeze(1)
#         # feature = sentence_feat[:, :].unsqueeze(1)

#         # input_char = torch.cat((initial_word, word, feature), 2)
#         # # print(input_char.size())

#         # print(initial_word.size())
#         predicted_output, hidden = decoder(features, hidden,initial_word,encoder_outputs)
...
#         # initial_word = hi
#         # hidden = hidden.unsqueeze(0)
#         # hidden2 = hidden.unsqueeze(1)
#         # hidden=hidden2
#         # print(hidden2.size())
#         # initial_word = encoder.embedding_layer(sentence[:,idx]).unsqueeze(1)
        
#         # initial_word = hidden.permute(1, 0, 2)
#         # print(initial_word.size())

#         # topv, topi = predicted_output.data.topk(1)
#         # initial_word = encoder.embedding_layer(torch.tensor(char_to_index[index_to_char[topi.item()]]).view(1, -1).cuda())
...
def main():
    ...
    s2s = torch.load('/home/amit-nlprl/Documents/samparkPipeline/Machine-Translation-System-master/saved.pt')
    test_file = BASE_DIR + 'hindi-test'
